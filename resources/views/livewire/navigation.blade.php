...
<nav x-data="{ open: @entangle('open') }"
	...
	<div class="flex flex-col md:flex-row items-center gap-x-7">
		...
		<div :class="open ? 'flex' : 'hidden'" 
     		class="md:flex lg:flex-grow flex-wrap items-start flex-col md:flex-row gap-x-6">
     
			<a href="{{ route('calendar') }}" class="flex items-center cursor-pointer py-3 gap-1 md:gap-[2px] md:py-0"
